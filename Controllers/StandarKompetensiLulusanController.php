<?php
/**
    @class StandarKompetensiLulusanControlelr
    @extends BaseController
    @description SKL Controller
*/
namespace Controllers;

class StandarKompetensiLulusanController extends \System\BaseController
{
    public function index()	
    {    
        // set judul halaman
        $this->viewBag->pageTitle="Standar Kompetensi Lulusan";      
        
        // $dataindikator = new \Models\SPMIModel(); 
        // $listdataindikator = $dataindikator->getIndikatorByIdStandar(1);

        // $this->viewBag->listdataindikator = $listdataindikator;

        $datapemetaan = new \Models\SPMIModel(); 
        $listdatapemetaan = $datapemetaan->getPemetaanMutu(1);

        $this->viewBag->listdatapemetaan = $listdatapemetaan;

        $datarencanapemenuhan = new \Models\SPMIModel(); 
        $listdatarencanapemenuhan = $datarencanapemenuhan->getRencanaPemenuhan(1);

        $this->viewBag->listdatarencanapemenuhan = $listdatarencanapemenuhan;

        $datapelaksanaanpemenuhan = new \Models\SPMIModel(); 
        $listdatapelaksanaanpemenuhan = $datapelaksanaanpemenuhan->getPelaksanaanPemenuhan(1);

        $this->viewBag->listdatapelaksanaanpemenuhan = $listdatapelaksanaanpemenuhan;

        $datamonev = new \Models\SPMIModel(); 
        $listdatamonev = $datamonev->getMonev(1);

        $this->viewBag->listdatamonev = $listdatamonev;

        $datastrategi = new \Models\SPMIModel(); 
        $listdatastrategi = $datastrategi->getStrategi(1);
    ...
    {
        $dataindikator = new \Models\SPMIModel(); 
        $listdataindikator = $dataindikator->getIndikatorByIdStandarAjax(1);

        return $this->Json($listdataindikator);
    }

    public function pemetaanmutulistajax()
    {
        // client side paging
        $dbusermanagement = new \Models\SPMIModel("pemetaanmutu");
        $users = $dbusermanagement->getPemetaanMutuDT(1);
    ...
    public function savePemetaan()
	{
		...
		$newdata = $datamodel->updateorcreatemodel(null, ['no_indikator', 'nama_indikator', 'comp_code', 'compcode']);
