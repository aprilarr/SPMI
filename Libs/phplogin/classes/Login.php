...
class Login
{
    ...
    {        
        ...
        $this->dataModel = new \Models\UsermanagementModel();
        ...
        // if user has an active session on the server
            ...
            if (isset($_POST["user_edit_submit_name"])) {
                ...
                $this->editUserName($_POST['comp_code'],$_POST['user_name']);
            ...
            // user try to change his password
                ...
                $this->editUserPassword($_POST['comp_code'],$_POST['user_password_old'], $_POST['user_password_new'], $_POST['user_password_repeat'],$_POST['captcha']);
            }
        ...
        // if user just submitted a login form
            ...
            $this->loginWithPostData($_POST['comp_code'],$_POST['user_name'], $_POST['user_password'], $_POST['user_rememberme']);
        }

        // checking if user requested a password reset mail
        if (isset($_POST["request_password_reset"]) && isset($_POST['user_name'])) {
            $this->setPasswordResetDatabaseTokenAndSendMail($_POST['user_name'], $_POST["captcha"]);
            ...
            $this->editNewPassword($_POST['comp_code'],$_POST['user_name'], $_POST['user_password_reset_hash'], $_POST['user_password_new'], $_POST['user_password_repeat']);
        }
     ...
     */
    private function getUserData($compcode,$user_name)
    {
        //$dataModel = new \Models\UsermanagementModel("users");
        return $this->dataModel->find("users", "user_name=?", [$user_name]);
    }
    ...
    {
        $domainusername = $_SERVER["PHP_AUTH_USER"];
        $username = explode("\\", $domainusername)[1];

        //$dataModel = new \Models\UsermanagementModel("users");
        $datauser = $this->dataModel->find("users", "user_name=?", [$username]);        
        if ($datauser != null)
        {
            $_SESSION['user_id'] = $datauser->id;
            $_SESSION['user_nik'] = $datauser->user_nik;
            $_SESSION['user_name'] = $datauser->user_name;       
            $_SESSION['user_fullname'] = $datauser->user_fullname;     
            $_SESSION['comp_code'] = $datauser->compcode;
            $_SESSION['user_email'] = $datauser->user_email;
            $_SESSION['user_logged_in'] = 1;

            // declare user id, set the login status to true
            $this->user_id = $datauser->id;
            $this->user_name = $datauser->user_name;
            $this->user_email = $datauser->user_email;
            $this->user_is_logged_in = true;

            return true;
        } else {
            // read otogeneral            
            $otoGeneral = new \Models\OTOGeneralEmployeeModel();
            $sql = "select * from tr_emp_Employee where ntusername=?";
            $otoGeneralData = $otoGeneral->getrow($sql, [$username]);            

            if ($otoGeneralData != null)
            {
                // register new user
                $datauser = $this->dataModel->create('users');
                $datauser->compcode = $otoGeneralData["CompID"]==1?"001":"002";
                $datauser->user_name = strtolower($otoGeneralData["NTUserName"]);
                $datauser->user_nik = $otoGeneralData["EmployeeID"];
                $datauser->user_fullname = $otoGeneralData["EmployeeName"];
                $datauser->user_password_hash = "";
                $datauser->user_email = $otoGeneralData["NTUserName"]."@oto.co.id";
                $datauser->user_activation_hash = "";
                $datauser->user_address = $otoGeneralData["Address"];
                $datauser->user_registration_ip = $_SERVER['REMOTE_ADDR'];
                $datauser->user_registration_datetime = date("Y-m-d H:i:s");
                $user_id = $this->dataModel->save($datauser);
    ...
    private function loginWithCookieData()
    {
        if (isset($_COOKIE['rememberme'])) {
            ...
            if ($hash == hash('sha256', $user_id . ':' . $token . COOKIE_SECRET_KEY) && !empty($token)) {
                ...
                //$dataModel = new \Models\UsermanagementModel("users");
                $datauser = $this->dataModel->find("users", "id=? and user_rememberme_token=? and compcode=? and user_rememberme_token is not null", [$user_id, $token]);

                if ($datauser != null)
                {
                    $_SESSION['user_id'] = $datauser->user_id;
                    $_SESSION['user_name'] = $datauser->user_name;
                    $_SESSION['user_code'] = $datauser->compcode;
                    $_SESSION['comp_code'] = $datauser->compcode;
                    $_SESSION['user_email'] = $datauser->user_email;
                    $_SESSION['user_logged_in'] = 1;
     ...
     */
    private function loginWithPostData($compcode,$user_name, $user_password, $user_rememberme)
    {        
        //$this->doLogout();
        //$dataModel = new \Models\UsermanagementModel("users");
        ...
        // if POST data (from login form) contains non-empty user_name and non-empty user_password
            ...
            if (!filter_var($user_name, FILTER_VALIDATE_EMAIL)) {
                ...
                $result_row = $this->getUserData($compcode,trim($user_name));
            // if user has typed a valid email address, we try to identify him with his user_email
            } else {                
                $result_row = $this->dataModel->find("users", "user_email=?", [$user_name]);
            }            
            ...
            // using PHP 5.5's password_verify() function to check if the provided passwords fits to the hash of that user's password
                ...
                $failedloginuser = $this->dataModel->find("users", "user_name=? or user_email=?", [$user_name, $user_name]);
                $failedloginuser->user_failed_logins = $failedloginuser->user_failed_logins + 1;
                $failedloginuser->user_last_failed_login = date("Y/m/d H:i:s");
                $this->dataModel->save($failedloginuser);
            ...
            // has the user activated their account with the verification email
                ...
                $_SESSION['comp_code'] = $result_row->compcode;
                $_SESSION['user_logged_in'] = 1;
                ...
                $successloginuser = $this->dataModel->find("users","id=?",[$result_row->id]);
                $successloginuser->user_failed_logins = 0;
                $successloginuser->user_last_failed_login = null;
                $this->dataModel->save($successloginuser);
                ...
                if (defined('HASH_COST_FACTOR')) {
                    ...
                    if (password_needs_rehash($result_row->user_password_hash, PASSWORD_DEFAULT, array('cost' => HASH_COST_FACTOR))) {
                        ...
                        $recalculatehashuser = $this->dataModel->find("users","id=?",[$result_row->id]);
                        $recalculatehashuser->user_password_hash = $user_password_hash;                        
                        $query_update = $this->dataModel->save($recalculatehashuser);
    ...
    private function newRememberMeCookie()
    {
        ...
        //if ($this->databaseConnection()) {
            ...
            $remembermeuser = $this->dataModel->this->find("users","id=?",[$_SESSION['user_id']]);
            $remembermeuser->user_rememberme_token = $random_token_string;                        
            $this->dataModel->save($remembermeuser);
    ...
    private function deleteRememberMeCookie()
    {
        ...
        $remembermeuser = $this->dataModel->find("users","id=?",[$_SESSION['user_id']]);
     ...
     */
    public function editUserName($compcode,$user_name)
    {
        //$dataModel = new \System\BaseModel("users");
        ...
        // TODO: maybe this pattern should also be implemented in Registration.php (or other way round)
            ...
            $result_row = $this->getUserData($compcode,$user_name);

            if (isset($result_row->user_id)) {
                $this->errors[] = MESSAGE_USERNAME_EXISTS;
            } else {
                // write user's new data into database
                $edituser = $this->dataModel->find("users","id=?",[$_SESSION['user_id']]);
                $edituser->user_name = $user_name;                        
                $query_edit_user_name = $this->dataModel->save($edituser);

                if ($query_edit_user_name != null) {
    ...
    public function editUserEmail($user_email)
    {
        ...
        // user mail cannot be empty and must be in email format
            ...
            $result_row = $this->dataModel->find("users","user_email=?",[$user_email]);            
            ...
            if (isset($result_row->id)) {
                ...
                $query_edit_user_email = $this->dataModel->save($edituser);

                if ($query_edit_user_email != null) {
     ...
     */
    public function editUserPassword($compcode,$user_password_old, $user_password_new, $user_password_repeat, $captcha)
    {
        if (strtolower($captcha) != strtolower($_SESSION['captcha'])) {
            $this->errors[] = MESSAGE_CAPTCHA_WRONG." -> ".$_SESSION['captcha']." <> ".$captcha;
        } elseif (empty($user_password_new) || empty($user_password_repeat) || empty($user_password_old)) {
            $this->errors[] = MESSAGE_PASSWORD_EMPTY;
        ...
        // all the above tests are ok
            ...
            $result_row = $this->getUserData($compcode,$_SESSION['user_name']);
            ...
            if (isset($result_row->user_password_hash)) {
                ...
                if (password_verify($user_password_old, $result_row->user_password_hash)) {
                    ...
                    //$dataModel = new \System\BaseModel("users");
                    $editpassworduser = $this->dataModel->find("users","id=?",[$_SESSION['user_id']]);
                    $editpassworduser->user_password_hash = $user_password_hash;                        
                    $query_update = $this->dataModel->save($editpassworduser);
     ...
     */
    public function setPasswordResetDatabaseTokenAndSendMail($user_name, $captcha, $compcode=null)
    {
        ...
        if (strtolower($captcha) != strtolower($_SESSION['captcha'])) {
            ...
            if (isset($result_row->id)) {
                // database query:
                //$dataModel = new \System\BaseModel("users");
                $resetuser = $this->dataModel->find("users","id=?",[$result_row->id]);
                ...
                if ($query_update) {
                    ...
                    $this->sendPasswordResetMail($compcode, $user_name, $result_row->user_email, $user_password_reset_hash);
                    return true;
                } else {
     ...
     */
    public function sendPasswordResetMail($compcode, $user_name, $user_email, $user_password_reset_hash)
    {
        $mail = new \PHPMailer\PHPMailer(); // create a new object
        $mail->IsSMTP(); // enable SMTP
        $mail->SMTPDebug = false; // debugging: 1 = errors and messages, 2 = messages only
        $mail->SMTPAuth = EMAIL_SMTP_AUTH; // authentication enabled
        $mail->SMTPSecure = EMAIL_SMTP_ENCRYPTION; // secure transfer enabled REQUIRED for Gmail
        ...
        $mail->SetFrom(EMAIL_PASSWORDRESET_FROM);
        ...
        $emailBody = "Anda telah melakukan permintaan reset password di ".SITE_TITLE.",<br>";        
        $emailBody .= EMAIL_PASSWORDRESET_CONTENT . '<br><br><a href="'.$link.'">' . $link.'</a>';
        ...
        $link    = EMAIL_PASSWORDRESET_URL.'?user_name='.urlencode($user_name).'&verification_code='.urlencode($user_password_reset_hash).'&compcode='.$compcode;

        $emailBody = "Anda telah melakukan permintaan reset password di ".SITE_TITLE." ".ucfirst($compcode).",<br>";
        //$emailBody .= "silahkan klik link dibawah ini untuk melakukan reset password.<br>";
        $mail->Body = $emailBody.EMAIL_PASSWORDRESET_CONTENT . '<br><a href="'.$link.'">' . $link.'</a>';
        $mail->IsHTML(true);
*/
        if(!$mail->Send()) {
     ...
     */
    public function editNewPassword($compcode, $user_name, $user_password_reset_hash, $user_password_new, $user_password_repeat)
    {
        // TODO: timestamp!
        $user_name = trim($user_name);
        ...
        } else //if ($this->databaseConnection()) 
        {
            ...
            // write users new hash into database
            //$dataModel = new \System\BaseModel("users");
            $newpassworduser = $this->dataModel->find("users","user_name=? and user_password_reset_hash=?",[$user_name,$user_password_reset_hash]);
            $newpassworduser->user_password_hash = $user_password_hash;                        
            $newpassworduser->user_password_reset_hash = NULL;                        
            $newpassworduser->user_password_reset_timestamp = null;                        
            $query_update = $this->dataModel->save($newpassworduser);
