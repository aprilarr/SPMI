...
class SQLServer extends AQueryWriter implements QueryWriter
{
    ...
    protected function constrain( $table, $table1, $table2, $property1, $property2 )
    {
        try {
            ...
            if ( $this->code( $columns[$property1] ) !== SQLServer::C_DATATYPE_UINT32 ) {
                $this->widenColumn( $table, $property1, SQLServer::C_DATATYPE_UINT32 );
            }

            if ( $this->code( $columns[$property2] ) !== SQLServer::C_DATATYPE_UINT32 ) {
                $this->widenColumn( $table, $property2, SQLServer::C_DATATYPE_UINT32 );
            }
    ...
    public function __construct( Adapter $adapter )
    {
        $this->typeno_sqltype = array(
            SQLServer::C_DATATYPE_BOOL             => ' TINYINT ',
            SQLServer::C_DATATYPE_UINT8            => ' TINYINT ',
            SQLServer::C_DATATYPE_UINT32           => ' INT ',
    ...
    public function scanType( $value, $flagSpecial = FALSE )
    {
        ...
        if ( !$this->startsWithZeros( $value ) ) {
            ...
            if ( is_numeric( $value ) && ( floor( $value ) == $value ) && $value >= 0 && $value <= 255 ) {
                return SQLServer::C_DATATYPE_UINT8;
            }
